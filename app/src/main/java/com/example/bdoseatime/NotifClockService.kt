				...
				if (real_seconds_since_reference_real_time >= 0) {
					...
					if ((remainder >= 0) and (remainder < 200 * 60)) {
						...
						if (last_state == NIGHT) {
							...
							val real_time_at_bdo_night = current_real_time.plusSeconds(real_seconds_to_bdo_night.toLong()).format(time_formatter)
							...
							val body: String = getString(R.string.day_notif_body, formatted_current_bdo_time, real_minutes_to_bdo_night.toInt(), real_remainder_seconds_to_bdo_night.toInt(), real_time_at_bdo_night, recoverable_energy.toInt())
						...
						if (last_state == DAY) {
							...
							val real_time_at_bdo_day = current_real_time.plusSeconds(real_seconds_to_bdo_day).format(time_formatter)
							...
							val body: String = getString(R.string.night_notif_body, formatted_current_bdo_time, real_minutes_to_bdo_day.toInt(), real_remainder_seconds_to_bdo_day.toInt(), real_time_at_bdo_day, recoverable_energy.toInt())
							val styled_body: Spanned = Html.fromHtml(body, FROM_HTML_MODE_LEGACY)
							builder.setContentText("It is $formatted_current_bdo_time now in BDO SEA")
								.setStyle(NotificationCompat.BigTextStyle()
									.bigText(styled_body))
